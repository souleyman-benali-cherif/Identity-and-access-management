...
class _EditUserScreenState extends ConsumerState<EditUserScreen> {
  ...
  Future<void> _findAlumniStudent() async {
    ...
    if (person == null ||
        (!person.uniqueId.startsWith('STU') &&
            !person.uniqueId.startsWith('PHD'))) {
      setState(() {
        _isLoading = false;
    ...
    if (person.isAlumni) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('This person is already marked as alumni.')));
      return;
    }
  ...
  Future<void> _submitAlumni() async {
    ...
    if (year == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('Graduation year must be a valid number.')));
      return;
    }
    if (year > DateTime.now().year) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('Graduation year cannot be in the future.')));
      return;
    }
    ...
    try {
      ...
      await showDialog(
        ...
        builder: (context) => AlertDialog(
          ...
          content: Text(
              '${_foundAlumniStudent!.firstName} ${_foundAlumniStudent!.lastName} has been marked as an alumni. Their ID remains ${_foundAlumniStudent!.uniqueId}.'),
          actions: [
            ElevatedButton(
              onPressed: () {
      ...
      setState(() {
        ...
        _foundAlumniStudent = null;
        _foundAlumniStudentDetails = null;
        _userType = AppConstants
            .typeUndergraduate; // Reset drop down to first item per instructions
      });
    } catch (e) {
      if (!mounted) return;
      setState(() => _isLoading = false);
      ...
      if (!_isEditing) {
        ...
        if (dup != null) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
                content: Text(
                    'Possible duplicate found: ${dup.uniqueId}. Please verify.')),
          );
          return;
        }
        ...
        // Email check
        final isUniqueEmail =
            await idService.isEmailUnique(_emailCtrl.text.trim());
        if (!isUniqueEmail) {
          if (!mounted) return;
          setState(() => _isLoading = false);
          ScaffoldMessenger.of(context).showSnackBar(
        ...
        if (_userType == AppConstants.typePhD) {
          final exists = await idService
              .checkProfessorExists(_supervisingProfCtrl.text.trim());
          if (!exists) {
            if (!mounted) return;
            setState(() => _isLoading = false);
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                  content: Text(
                      'Supervising Professor ID is invalid or does not exist.')),
            );
            return;
          }
        }
        ...
        final uniqueId =
            await IdGenerator.generateUniqueId(_userType, DateTime.now().year);
        ...
        if (isStudent) {
          await idService.saveStudent(StudentModel(
            ...
            supervisingProfessorId: _userType == AppConstants.typePhD
                ? _supervisingProfCtrl.text.trim()
                : null,
            stayStartDate: _userType == AppConstants.typeInternational
                ? _stayStartCtrl.text.trim()
                : null,
            stayEndDate: _userType == AppConstants.typeInternational
                ? _stayEndCtrl.text.trim()
                : null,
          ));
        } else if (isFaculty) {
          await idService.saveFaculty(FacultyModel(
            ...
            secondaryDepartments: _secondaryDeptsCtrl.text
                .split(',')
                .map((e) => e.trim())
            ...
            researchAreas: _researchAreasCtrl.text
                .split(',')
                .map((e) => e.trim())
                .where((e) => e.isNotEmpty)
                .toList(),
            ...
            contractEndDate: (_contractType == 'Permanent')
                ? null
                : _contractEndCtrl.text.trim(),
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            const Text(
                'The user has been created successfully. Please securely share the following credentials:'),
  ...
  Widget _buildAlumniFlow() {
    return Column(
      ...
      children: [
        const Text('STEP 2 — SEARCH FOR EXISTING STUDENT',
            ...
            Expanded(
              child: TextField(
                ...
                decoration: const InputDecoration(
                  ...
                  helperText:
                      "Enter the student's STU or PHD ID to find their record.",
                ),
              ),
            ),
          ...
          Card(
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  Text(
                      'Found Student: ${_foundAlumniStudent!.firstName} ${_foundAlumniStudent!.lastName}',
                      style: const TextStyle(
                  ...
                  Text(
                      'Entry Year: ${_foundAlumniStudentDetails?.entryYear ?? "N/A"}'),
                  Text(
                      'Faculty & Dept: ${_foundAlumniStudentDetails?.faculty ?? "N/A"} / ${_foundAlumniStudentDetails?.department ?? "N/A"}'),
                ],
              ),
            ),
          ),
          const SizedBox(height: 32),
          const Text('STEP 4 — ENTER GRADUATION YEAR',
              style: TextStyle(fontWeight: FontWeight.bold)),
          const SizedBox(height: 16),
          TextField(
            controller: _alumniGradYearCtrl,
            decoration: const InputDecoration(
                labelText: 'Graduation year', hintText: 'e.g. 2024'),
  ...
  Widget _buildStudentFields() {
    return Column(
      ...
      children: [
        ...
        const Text('Student Specific Info',
        ...
        Row(
          children: [
            Expanded(
                child: TextFormField(
                    controller: _diplomaTypeCtrl,
                    decoration:
                        const InputDecoration(labelText: 'HS Diploma Type'),
                    validator: Validators.required)),
            const SizedBox(width: 16),
            Expanded(
                child: TextFormField(
                    controller: _diplomaYearCtrl,
                    decoration:
                        const InputDecoration(labelText: 'HS Diploma Year'),
                    validator: (v) => Validators.notFutureYear(
                        int.tryParse(v ?? ''),
                        fieldLabel: 'Diploma Year'))),
            const SizedBox(width: 16),
            Expanded(
                child: DropdownButtonFormField<String>(
              initialValue: _diplomaHonors,
              decoration: const InputDecoration(labelText: 'HS Diploma Honors'),
              items: AppConstants.diplomaHonors
          ...
          children: [
            Expanded(
                child: TextFormField(
                    ...
                    decoration:
                        const InputDecoration(labelText: 'Chosen Major'),
                    validator: Validators.required)),
            const SizedBox(width: 16),
            Expanded(
                child: TextFormField(
                    controller: _entryYearCtrl,
                    decoration: const InputDecoration(labelText: 'Entry Year'),
                    validator: (v) => Validators.notFutureYear(
                        int.tryParse(v ?? ''),
                        fieldLabel: 'Entry Year'))),
          ],
        ),
        const SizedBox(height: 16),
        Row(
        ...
        if (_userType == AppConstants.typePhD) ...[
          const SizedBox(height: 16),
          TextFormField(
            controller: _supervisingProfCtrl,
            decoration: const InputDecoration(
                labelText: 'Supervising Professor ID (starts with FAC)'),
            validator: Validators.professorIdPrefix,
          ),
        ],
        if (_userType == AppConstants.typeInternational) ...[
          ...
          Row(
            children: [
              Expanded(
                child: TextFormField(
                  controller: _stayStartCtrl,
                  decoration: const InputDecoration(
                      labelText: 'Stay Start Date (YYYY-MM-DD)'),
                  validator: Validators.validDateFormat,
                ),
              ),
              const SizedBox(width: 16),
              Expanded(
                child: TextFormField(
                  controller: _stayEndCtrl,
                  decoration: const InputDecoration(
                      labelText: 'Stay End Date (YYYY-MM-DD)'),
                  validator: (v) => Validators.stayDurationFromStrings(
                      _stayStartCtrl.text, v),
                ),
              ),
            ],
          )
  ...
  Widget _buildFacultyFields() {
    return Column(
      ...
      children: [
        const SizedBox(height: 24),
        const Text('Faculty Specific Info',
            style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
        const Divider(),
        Row(
          children: [
            Expanded(
              ...
              items: AppConstants.facultyRanks
                  .map((e) => DropdownMenuItem(value: e, child: Text(e)))
                  .toList(),
            ...
            Expanded(
                child: TextFormField(
                    controller: _empCategoryCtrl,
                    decoration:
                        const InputDecoration(labelText: 'Employment Category'),
                    validator: Validators.required)),
          ],
        ),
        const SizedBox(height: 16),
        TextFormField(
          controller: _apptStartCtrl,
          decoration: const InputDecoration(
              labelText: 'Appointment Start Date (YYYY-MM-DD)'),
          validator: Validators.validDateFormat,
        ),
        const SizedBox(height: 16),
        Row(
          children: [
            Expanded(
                child: TextFormField(
                    controller: _primaryDeptCtrl,
                    decoration:
                        const InputDecoration(labelText: 'Primary Department'),
                    validator: Validators.required)),
            const SizedBox(width: 16),
            Expanded(
              child: TextFormField(
                controller: _secondaryDeptsCtrl,
                decoration: const InputDecoration(
                    labelText: 'Secondary Departments (comma-separated)',
                    hintText: 'Optional'),
                validator: (v) {
                  if (v == null || v.trim().isEmpty) return null; // Optional
                  final parts = v.split(',').map((e) => e.trim()).toList();
                  if (parts.any((e) => e.isEmpty)) {
                    return 'Remove empty entries (no commas without text).';
                  }
                  return null;
                },
              ),
        ...
        Row(
          children: [
            Expanded(
                child: TextFormField(
                    controller: _officeBldgCtrl,
                    decoration:
                        const InputDecoration(labelText: 'Office Building'),
                    validator: Validators.required)),
            const SizedBox(width: 16),
            Expanded(
                child: TextFormField(
                    controller: _officeFloorCtrl,
                    decoration:
                        const InputDecoration(labelText: 'Office Floor'),
                    validator: Validators.required)),
            const SizedBox(width: 16),
            Expanded(
                child: TextFormField(
                    controller: _officeRoomCtrl,
                    decoration: const InputDecoration(labelText: 'Office Room'),
                    validator: Validators.required)),
          ],
        ),
        const SizedBox(height: 16),
        Row(
          children: [
            Expanded(
                child: TextFormField(
                    controller: _phdInstCtrl,
                    decoration:
                        const InputDecoration(labelText: 'PhD Institution'),
                    validator: Validators.required)),
            const SizedBox(width: 16),
            Expanded(
              child: TextFormField(
                controller: _researchAreasCtrl,
                decoration: const InputDecoration(
        ...
        Row(
          children: [
            ...
            if (_contractType != 'Permanent') ...[
              ...
              Expanded(
                child: TextFormField(
                  controller: _contractEndCtrl,
                  decoration:
                      const InputDecoration(labelText: 'Contract End Date'),
                  validator: (v) => Validators.endDateAfterStartDate(
                      v, _contractStartCtrl.text,
        ...
        const SizedBox(height: 16),
        TextFormField(
            controller: _teachingHoursCtrl,
            decoration:
                const InputDecoration(labelText: 'Teaching Hours per Week'),
            keyboardType: TextInputType.number,
            validator: (v) => Validators.numberInRange(
                int.tryParse(v ?? ''), 1, 100,
                fieldLabel: 'Hours')),
      ],
    );
  }
  ...
  Widget _buildStaffFields() {
    return Column(
      ...
      children: [
        const SizedBox(height: 24),
        const Text('Staff Specific Info',
            style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
        const Divider(),
        Row(
          children: [
            Expanded(
                child: TextFormField(
                    controller: _staffDeptCtrl,
                    decoration:
                        const InputDecoration(labelText: 'Assigned Department'),
                    validator: Validators.required)),
            const SizedBox(width: 16),
            Expanded(
                child: TextFormField(
        ...
        Row(
          children: [
            ...
            Expanded(
              child: TextFormField(
                controller: _dateOfEntryCtrl,
                decoration: const InputDecoration(
                    labelText: 'Date of Entry (YYYY-MM-DD)'),
                validator: (v) {
                  if (v == null || v.isEmpty) {
                    return 'Date of entry is required';
                  }
                  try {
                    final dt = DateTime.parse(v);
                    return Validators.notFutureDate(dt,
                        fieldLabel: 'Date of entry');
                  } catch (_) {
  ...
  Widget _buildContractorFields() {
    return Column(
      ...
      children: [
        const SizedBox(height: 24),
        const Text('Contractor Specific Info',
            style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
        const Divider(),
        TextFormField(
            controller: _purposeCtrl,
            decoration: const InputDecoration(labelText: 'Contract Purpose'),
            validator: Validators.required),
        const SizedBox(height: 16),
        TextFormField(
          controller: _expiryCtrl,
          decoration: const InputDecoration(
              labelText: 'Expiry Date (YYYY-MM-DD)', hintText: '2026-12-31'),
          validator: (v) {
      ...
      body: Center(
        child: SingleChildScrollView(
          ...
          child: ConstrainedBox(
            constraints:
                const BoxConstraints(maxWidth: 800), // widened for larger form
            child: Card(
              child: Padding(
                ...
                child: Form(
                  ...
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.stretch,
                    children: [
                      Text(_isEditing ? 'Update Details' : 'Registration Form',
                          style: Theme.of(context).textTheme.headlineSmall),
                      const SizedBox(height: 24),
                      if (!_isEditing) ...[
                        const Text('STEP 1 — SELECT TYPE',
                            style: TextStyle(fontWeight: FontWeight.bold)),
                        const SizedBox(height: 16),
                      ...
                      if (_userType == AppConstants.typeAlumni && !_isEditing)
                            ...
                            Expanded(
                              child: TextFormField(
                                ...
                                validator: (v) {
                                  ...
                                  try {
                                    final dt = DateTime.parse(v);
                                    return Validators.dateOfBirth(dt,
                                        checkStudentAge: isStudentType);
                                  } catch (_) {
                        ...
                        if (!_isEditing) ...[
                          ...
                          if (_userType == AppConstants.typeContractor)
